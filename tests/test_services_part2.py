...
class TestUserProfileService:
    def test_get_user_profile(self):
        ...
        mock_db.collection().document().get.return_value.to_dict.return_value = {"email": "test@test.com"}
...
class TestMapsService:
    ...
    def test_find_polling_booth(self, mock_get):
        service = MapsService()
        service.api_key = "key"
        mock_get.return_value.json.return_value = {"status": "OK", "results": [{"name": "Booth 1", "geometry": {"location": {"lat": 1, "lng": 1}}, "vicinity": "V"}]}
        res = service.find_polling_booth(1, 1)
        assert res["booth_name"] == "Booth 1"

# Translate Service
from services.translate_service import TranslateService, ElectionContentTranslator
class TestTranslateService:
    def test_translate(self):
        service = TranslateService()
        service._initialized = True
        mock_client = MagicMock()
        mock_client.translate.return_value = {"translatedText": "T", "detectedSourceLanguage": "en"}
...
from services.tts_service import TextToSpeechService, AudioGuidancePlayer
class TestSpeechServices:
    @patch.dict('sys.modules', {'google.cloud.speech': MagicMock()})
    def test_recognize(self):
        # We need to re-import or reload to let the local import see the mocked module
        from services.speech_service import SpeechToTextService
