...
class TestAnalyticsService:
    ...
    def test_log_event_with_client_exception(self, mock_config):
        ...
        with patch.object(service, "_log_to_firestore", return_value=True) as mock_fallback:
            result = service.log_event("test_event", {"param": "value"})
            assert result is True
    ...
    def test_specific_events(self):
        ...
        with patch.object(service, "log_event", return_value=True) as mock_log:
            ...
            mock_log.assert_called_with("page_view", {"page_name": "home", "user_id": "u1"})

            service.log_feature_use("search", "u1")
            mock_log.assert_called_with("feature_use", {"feature_name": "search", "user_id": "u1"})

            service.log_language_change("hi", "u1")
            mock_log.assert_called_with("language_change", {"language": "hi", "user_id": "u1"})

            service.log_reminder_create("election", "u1")
            mock_log.assert_called_with("reminder_create", {"reminder_type": "election", "user_id": "u1"})

            service.log_calendar_sync(True, "u1")
            mock_log.assert_called_with("calendar_sync", {"success": True, "user_id": "u1"})
            ...
            mock_log.assert_called_with("voice_input", {"success": True, "user_id": "u1"})

            service.log_audio_playback("guidance", "u1")
            mock_log.assert_called_with("audio_playback", {"content_type": "guidance", "user_id": "u1"})

            service.log_ai_chat("hello", "u1")
            mock_log.assert_called_with("ai_chat", {"query_preview": "hello", "user_id": "u1"})
