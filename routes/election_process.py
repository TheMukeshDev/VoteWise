...
@election_process_bp.route("/", methods=["GET"])
@require_admin
def get_election_processes():
    """Get all election processes (admin)."""
    try:
        language = request.args.get("language")

        processes = election_process_service.get_all_for_admin()
...
@require_admin
def get_election_process(process_id):
    """Get a specific election process (admin)."""
    try:
        process = election_process_service.get_by_id(process_id)
...
def create_election_process():
    ...
    try:
        ...
        process = election_process_service.create(
            title=data["title"],
            intro=data["intro"],
            steps=data["steps"],
            tips=data.get("tips"),
...
@require_admin
def update_election_process(process_id):
    ...
    try:
        ...
        process = election_process_service.update(process_id, data)
...
@require_admin
def delete_election_process(process_id):
    """Delete an election process (admin only)."""
    try:
        success = election_process_service.delete(process_id)
